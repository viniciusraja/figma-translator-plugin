    ...
    run(fn, params) {
        return new Promise((resolve, reject) => {
            ...
            const cb = (event) => {
                ...
                if (((_a = event.data.pluginMessage) === null || _a === void 0 ? void 0 : _a.type) === "EVAL_RESULT") {
                    if (event.data.pluginMessage.id === id) {
                        window.removeEventListener("message", cb);
                        resolve(event.data.pluginMessage.result);
                    }
                }
                if (((_b = event.data.pluginMessage) === null || _b === void 0 ? void 0 : _b.type) === "EVAL_REJECT") {
                    if (event.data.pluginMessage.id === id) {
                        window.removeEventListener("message", cb);
                        const message = event.data.pluginMessage.error;
                        reject(new Error(typeof message === "string"
                            ? message
